...
public class EnterpriseServlet extends HttpServlet {
    ...
    /**
     * @description: 从数据库获取数据进行认证页面数据的回显,每次信息认证页面加载都会发送get请求，将用户信息和企业信息分别放在objects[0]和objects[1]上，用户第一次登录时，因为没有认证所以无法查询到对应的企业信息，根据objects[1]是否为null判断该用户是否进行了认证，并且是否进行数据回显。
     * @return: void
